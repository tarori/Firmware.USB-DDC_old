...
uint8_t dg8saqFunctionSetup(uint8_t type, uint16_t wValue, uint16_t wIndex, U8 *Buffer)
{
	...
	****************************************************/
    ...
    case 0x52:
        ...
        if (gpio_get_pin_value(GPIO_CW_KEY_1))
            Buffer[0] |= REG_CWSHORT;
        if (gpio_get_pin_value(GPIO_CW_KEY_2))
            Buffer[0] |= REG_CWLONG;
        if (gpio_get_pin_value(GPIO_PTT_INPUT))
            Buffer[0] |= REG_PTT_INPUT;
        if (gpio_get_pin_value(PTT_1))
            Buffer[0] |= REG_PTT_1;
        if (gpio_get_pin_value(PTT_2))
            Buffer[0] |= REG_PTT_2;
        if (gpio_get_pin_value(PTT_3))
            Buffer[0] |= REG_PTT_3;
        if (TX_state)
            Buffer[0] |= REG_TX_state;
