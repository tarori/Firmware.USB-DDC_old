                     ...
                     char *lcd0, char *lcd1, char *lcd2, char *lcd3)
{
    ...
    if (lines == 4) {
        if (current_choice == menu_size - 1)
            x = 1;
        else if (current_choice == menu_size - 2)
            x = 0;
        else
            x = current_choice + 2;
...
#define VFO_VALUES 6
    ...
    if (LCD_upd == FALSE) {
        ...
        if (current_selection < 0)
            current_selection = VFO_VALUES - 1;
...
void pcf8574_addr_menu_level2()
{
    ...
    if (LCD_upd == FALSE) {
        ...
        PCFSELUP:
            ...
            if ((pcf8574_selected <= 0x20) && (i2c.pcf0x20))
                pcf8574_selected = 0x20;
            else if ((pcf8574_selected <= 0x21) && (i2c.pcf0x21))
                pcf8574_selected = 0x21;
            else if ((pcf8574_selected <= 0x22) && (i2c.pcf0x22))
                pcf8574_selected = 0x22;
            else if ((pcf8574_selected <= 0x23) && (i2c.pcf0x23))
                pcf8574_selected = 0x23;
            else if ((pcf8574_selected <= 0x24) && (i2c.pcf0x24))
                pcf8574_selected = 0x24;
            else if ((pcf8574_selected <= 0x25) && (i2c.pcf0x25))
                pcf8574_selected = 0x25;
            else if ((pcf8574_selected <= 0x26) && (i2c.pcf0x26))
                pcf8574_selected = 0x26;
            else if ((pcf8574_selected <= 0x27) && (i2c.pcf0x27))
                pcf8574_selected = 0x27;
            else if ((pcf8574_selected <= 0x38) && (i2c.pcf0x38))
                pcf8574_selected = 0x38;
            else if ((pcf8574_selected <= 0x39) && (i2c.pcf0x39))
                pcf8574_selected = 0x39;
            else if ((pcf8574_selected <= 0x3a) && (i2c.pcf0x3a))
                pcf8574_selected = 0x3a;
            else if ((pcf8574_selected <= 0x3b) && (i2c.pcf0x3b))
                pcf8574_selected = 0x3b;
            else if ((pcf8574_selected <= 0x3c) && (i2c.pcf0x3c))
                pcf8574_selected = 0x3c;
            else if ((pcf8574_selected <= 0x3d) && (i2c.pcf0x3d))
                pcf8574_selected = 0x3d;
            else if ((pcf8574_selected <= 0x3e) && (i2c.pcf0x3e))
        ...
        PCFSELDN:
            // Select an available PCF8574
            if ((pcf8574_selected >= 0x3f) && (i2c.pcf0x3f))
                pcf8574_selected = 0x3f;
            else if ((pcf8574_selected >= 0x3e) && (i2c.pcf0x3e))
                pcf8574_selected = 0x3e;
            else if ((pcf8574_selected >= 0x3d) && (i2c.pcf0x3d))
...
void bias_raw_menu_level2(void)
{
    ...
    if (menu_level == BIAS_CASE3_MENU)
        current_selection = cdata.cal_LO;
    else if (menu_level == BIAS_CASE4_MENU)
        current_selection = cdata.cal_HI;
...
void bias_autoset_menu_level2(void)
{
    ...
    if (menu_level == BIAS_CASE0_MENU)
        current_selection = cdata.Bias_LO;
    else /*if (menu_level == BIAS_CASE1_MENU)*/
        current_selection = cdata.Bias_HI;
...
void temperature_menu_level2(void)
{
    ...
    if (menu_level == TMP_CASE0_MENU)
        current_selection = cdata.hi_tmp_trigger;
    else if (menu_level == TMP_CASE1_MENU)
        current_selection = cdata.Fan_On;
    else if (menu_level == TMP_CASE2_MENU)
        current_selection = cdata.Fan_Off;
...
void filters_adjust_menu_level3(void)
{
    ...
    if (menu_level == FILTER_BPF_ADJUST_MENU)
        current_selection = cdata.FilterCrossOver[menu_data];
    else
        current_selection = cdata.TXFilterCrossOver[menu_data];
    ...
    if (LCD_upd == FALSE) {
        ...
        // Store Current value in running storage
        if (menu_level == FILTER_BPF_ADJUST_MENU)
            cdata.FilterCrossOver[menu_data] = current_selection;
        else
            cdata.TXFilterCrossOver[menu_data] = current_selection;
...
void i2c_menu_level2(void)
{
    ...
    if (FIRST_run == FALSE) // Fetch values if first time
    {
        if (menu_level == I2C_SI570_MENU)
            ...
            current_selection = cdata.PCF_I2C_Mobo_addr;
        else if (menu_level == I2C_PCF8574_LPF1_MENU)
            current_selection = cdata.PCF_I2C_lpf1_addr;
        else if (menu_level == I2C_PCF8574_LPF2_MENU)
            current_selection = cdata.PCF_I2C_lpf2_addr;
        else if (menu_level == I2C_PCF8574_FAN_MENU)
            current_selection = cdata.PCF_I2C_Ext_addr;
        else if (menu_level == I2C_TMP100_MENU)
            current_selection = cdata.TMP100_I2C_addr;
        else if (menu_level == I2C_AD5301_MENU)
            current_selection = cdata.AD5301_I2C_addr;
        ...
        //
            ...
            if (menu_manage == 0) {
                ...
                if (menu_level == 0)
                    ...
                    pswr_menu();
                else if (menu_level == PSWR_CASE0_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE1_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE2_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE3_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE4_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE5_MENU)
                    pswr_menu();
                else if (menu_level == PSWR_CASE6_MENU)
                    pswr_menu();
