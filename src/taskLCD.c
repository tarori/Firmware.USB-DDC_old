...
void ___writeNibbleToLCD(uint8_t selectedRegister, uint8_t nibble)
{
    ...
    if (nibble & 0x01)
        gpio_set_gpio_pin(LCD_D4);
    if (nibble & 0x02)
        gpio_set_gpio_pin(LCD_D5);
    if (nibble & 0x04)
        gpio_set_gpio_pin(LCD_D6);
    if (nibble & 0x08)
        gpio_set_gpio_pin(LCD_D7);
...
void ___writeByteToLCD(uint8_t selectedRegister, uint8_t byte)
{
    ...
    if (selectedRegister == DATA_REGISTER) {
        if (FEATURE_LCD_HD44780) {
            ...
            switch (position) {
            case 0:
                ___writeByteToLCD(COMMAND_REGISTER, 0x80);
                vTaskDelay(20);
                break;

            case 20:
                ___writeByteToLCD(COMMAND_REGISTER, 0xC0);
                vTaskDelay(20);
                break;

            case 40:
                ___writeByteToLCD(COMMAND_REGISTER, 0x94);
                vTaskDelay(20);
                break;

            case 60:
                ___writeByteToLCD(COMMAND_REGISTER, 0xd4);
                vTaskDelay(20);
                break;
            ...
            case 20:
                ___writeByteToLCD(COMMAND_REGISTER, 0xA0);
                vTaskDelay(20);
                break;
            ...
            case 60:
                ___writeByteToLCD(COMMAND_REGISTER, 0xE0);
                vTaskDelay(20);
                break;
...
void ___lcd_puthex(U8 c)
{
    ...
    cl = c & 0x0f;
    if (ch < 10)
        ___writeByteToLCD(DATA_REGISTER, (ch + '0'));
    else
        ___writeByteToLCD(DATA_REGISTER, (ch + '0' + 7));
    if (cl < 10)
        ___writeByteToLCD(DATA_REGISTER, (cl + '0'));
    else
        ___writeByteToLCD(DATA_REGISTER, (cl + '0' + 7));
}
        ...
        if (xStatus == pdPASS) {
            if (!FEATURE_LCD_NONE) {
                ...
                case lcdINIT:
                    ...
                    ___writeNibbleToLCD(COMMAND_REGISTER, 0x03);
                    vTaskDelay(50);
                    ___writeNibbleToLCD(COMMAND_REGISTER, 0x03);
                    vTaskDelay(50);
                    ___writeNibbleToLCD(COMMAND_REGISTER, 0x03);
                    vTaskDelay(50);
                    ___writeNibbleToLCD(COMMAND_REGISTER, 0x02);
                    vTaskDelay(50);
                ...
                case lcdHOME:
                    ___writeByteToLCD(COMMAND_REGISTER, (1 << LCD_HOME));
                    position = 0;
                    vTaskDelay(120);
                    break;
                ...
                case lcdGOTO:
                    position = (lcdQUEDATA.data.scrnPOS.row * 20) + lcdQUEDATA.data.scrnPOS.col;
